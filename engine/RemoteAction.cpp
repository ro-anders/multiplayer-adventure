...
PortcullisStateAction::PortcullisStateAction(void * ooga, int inPortPkey, int inNewSate, bool inActive) :
RemoteAction(CODE),
portPkey(inPortPkey),
newState(inNewSate),
isActive(inActive) {}
...
int PortcullisStateAction::serialize(char* buffer, int bufferLength) {
    ...
    int numChars = sprintf(buffer, "GS %d %d %d %d",
                           sender, portPkey, newState, (int)isActive);
    return numChars;
}

void PortcullisStateAction::deserialize(const char *message) {
    char type[8];
    sscanf(message, "%s %d %d %d %d", type, &sender, &portPkey, &newState, &isActive);
}
